...
struct PlayerView: View {
  ...
  /// The player's visual hierarchy: the video surface, the optional docked
  /// chat pane, and the transient overlays (raid / outgoing-raid / still-
  /// watching / go-live banners and the sleeping dim). Event and lifecycle
  /// handling is applied in `body` via the helpers in
  /// PlayerView+EventHandlers.swift; this property is purely the view tree.
  ...
  var body: some View {
    // `body` is deliberately a short pipeline. The player has ~30 lifecycle
    // and event modifiers; as one chain they exceed the Swift type-checker's
    // solver budget ("unable to type-check this expression in reasonable
    // time"). Grouping them into the small, focused helpers below (defined in
    // PlayerView+EventHandlers.swift) keeps each a fast, independently checked
    // expression. Helpers are applied top-to-bottom in the original order, so
    ...
    let withBanners = raidAndSleepBannerHandlers(stack)
    let withLifecycle = playbackLifecycleHandlers(withBanners)
    let withCommands = remoteCommandHandlers(withLifecycle)
    let withFocus = focusManagementHandler(withCommands)
    let withToggles = simulcastToggleHandlers(withFocus)
